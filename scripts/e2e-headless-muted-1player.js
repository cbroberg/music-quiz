/**
 * E2E Headless Muted 1-Player Test
 *
 * Minimal headless test: 1 browser, 1 player, MUTE_ALL mode.
 * Designed for stability — no multi-browser crashes.
 * Grows the question bank with every run.
 *
 * Usage:
 *   node scripts/e2e-headless-muted-1player.js             # 10 questions
 *   node scripts/e2e-headless-muted-1player.js 50          # 50 questions
...
async function main() {
  log(`🎮 E2E 1-Player Headless — ${QUESTIONS} questions, timer ${TIMER}s\n`);
  log(`📁 Output: ${outDir}`);

  // ─── Single browser, 2 contexts (host + player) ──
  ...
  const names = ['Christian'];
  const avatarIdx = [0];
  const playerPages = [playerPage];
  const allBrowsers = [browser];

  log(`🖥️ Mode: headless 1-player | ${QUESTIONS} questions`);

  hostPage.on('crash', () => log('💥 HOST PAGE CRASHED'));
  hostPage.on('pageerror', (err) => log(`💥 Host JS error: ${err.message?.slice(0, 100)}`));
  playerPage.on('crash', () => log('💥 PLAYER CRASHED'));
  playerPage.on('pageerror', (err) => log(`💥 Player JS error: ${err.message?.slice(0, 100)}`));
  ...
  await sleep(1000);
  await screenshot(hostPage, '02-lobby');
  log('');
  ...
  for (let q = 1; q <= QUESTIONS; q++) {
    ...
    try {
      ...
      log(`   ⚠️ Question ${q} didn't appear — checking state... (${waitErr?.message?.slice(0, 80)})`);
      try { await screenshot(hostPage, `Q${String(q).padStart(3,'0')}-host-timeout`); } catch {}
      try { await screenshot(playerPages[0], `Q${String(q).padStart(3,'0')}-p1-timeout`); } catch {}
      break;
    }
    await sleep(300);
    ...
    if (q < QUESTIONS) {
      try {
        ...
        log(`   ⚠️ Timeout waiting for Q${q + 1} (${transErr?.message?.slice(0, 80)})`);
        await sleep(2000);
      }
    }
  }
  ...
  await screenshot(playerPage, 'final-christian-result');
