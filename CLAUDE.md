...
### Outstanding issues
1. **MusicKit entitlement is currently DISABLED** in
   `apps/tvos/MusicQuiz/Config/MusicQuiz.entitlements`. The App ID at
   developer.apple.com has MusicKit + ShazamKit enabled, AND a Media ID
   `media.dk.webhouse.music-quiz` is registered, but the provisioning
   profile generated by `sigh` does NOT include the
   `com.apple.developer.musickit` entitlement when build is signed.
   Apple's API doesn't error — it just silently omits the entitlement,
   so xcodebuild fails with "Entitlement com.apple.developer.musickit
   not found and could not be included in profile". For first TestFlight
   build we shipped without it; **native music playback is non-functional
   on real Apple TV hardware until this is fixed**. Likely fixes to try:
   - Explicitly link the Media ID to the App ID via developer.apple.com
     (some Apple flows require this manual step)
   - Re-toggle MusicKit on the App ID and wait 5+ min for propagation
   - Contact Apple Developer support if neither works
